        ...
        <>
            ...
            <div className="shopsList">
                {shopDataContext.shopDataState.data && shopDataContext.shopDataState.data.map(e =>
                    <div className="item" key={`${e.location}_name`}>
                        <span onClick={() => {
                            setmapCenter({ lat: e.lat, lng: e.lng })
                        }}> {e.location}</span>
                        <button onClick={() => shopDataContext.deleteShop(e.location)}>
                ...
                <button className="addShopBtn" onClick={() => setshowModal(true)}>Add Shop</button>
            </div>
            {showModal &&
                <div className={`addShopModal ${showModal && "active"}`}>
